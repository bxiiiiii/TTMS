...
int Sale_Perst_SelectByUsrID(sale_list_t list, int usrID, user_date_t curDate, int ice) 
{
	...
	while(!feof(fp))
	{
		if(fread(&data, sizeof(sale_t), 1, fp)> 0)
		{
			if(data.user_id == usrID)
			{
				if(ice == 1 && DateCmp(data.date, curDate) == 0)
				{
					p = (sale_list_t)malloc(sizeof(sale_node_t));
					p->data = data;
					List_AddTail(list, p);
				}
				else if(ice == 2 && data.date.month == curDate.month)
			    {
					p = (sale_list_t)malloc(sizeof(sale_node_t));
					p->data = data;
					List_AddTail(list, p);
...
int Sale_Perst_SelectByUsrID_1(sale_list_t list, int usrID, user_date_t stDate, user_date_t enDate) 
{
	...
	while(!feof(fp))
	{
		if(fread(&data, sizeof(sale_t), 1, fp)> 0)
		{
			if(data.user_id == usrID)
			{
				if(DateCmp(data.date, stDate) && DateCmp(enDate, data.date))
				{
					p = (sale_list_t)malloc(sizeof(sale_node_t));
					p->data = data;
					List_AddTail(list, p);
