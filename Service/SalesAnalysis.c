...
int Salesanalysis_Srv_CompSaleVal(int usrID, user_date_t curdate, int ice)
{
	int amount=0;
	sale_list_t list;
	sale_node_t *pos;
	List_Init(list, sale_node_t); 
	Sale_Perst_SelectByUsrID(list,usrID,curdate,ice);
...
int Salesanalysis_Srv_CompSaleVal_1(int usrID, user_date_t stdate, user_date_t endate)
{
	int amount=0;
	sale_list_t list,pos;
	List_Init(list, sale_node_t);
	Sale_Perst_SelectByUsrID_1(list,usrID,stdate,endate);
	List_ForEach(list, pos)
	{
		amount+=pos->data.value;
	}
	return amount;
}
